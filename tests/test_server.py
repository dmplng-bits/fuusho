...
(fan-out isolation), and the full HTTP API in backend/routes.py.
...
from backend import apns_client, notification_dispatch, pairing, state_store  # noqa: E402
from backend.routes import api_blueprint  # noqa: E402
...
class TestPairingSession:
    """Unit-level tests for backend/pairing.py's ECDH handshake, independent
...
class TestNotifyEndpoint:
    def test_notify_dispatches_and_reports_ok(self, api_client, monkeypatch):
        dispatched = []
        monkeypatch.setattr(
            "backend.routes.notification_dispatch.dispatch_notification",
    ...
    def test_notify_reports_total_failure_as_502(self, api_client, monkeypatch):
        monkeypatch.setattr(
            "backend.routes.notification_dispatch.dispatch_notification", lambda *a: False,
        )
    ...
    def test_notify_internal_secret_enforced_when_configured(self, api_client, monkeypatch):
        monkeypatch.setattr("backend.routes.INTERNAL_API_SECRET", "scheduler-only")
        monkeypatch.setattr(
            "backend.routes.notification_dispatch.dispatch_notification", lambda *a: True,
        )
