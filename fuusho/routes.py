...
Flask Blueprint: the entire fuusho HTTP API.
  ...
  POST   /api/pair/complete    -> finish pairing: decrypt the app's
                                  ...
                                  device (see fuusho/pairing.py for the
                                  full handshake)
...
actually authenticates it. See fuusho/pairing.py for the security model.
...
def public_server_url():
    """Where the app should point after scanning — the process only knows
    its own internal host, not the LAN/public address people actually
    reach it at. Read per-request so embedders can set the env var at any
    point before first use; import order doesn't matter."""
    return os.environ.get("PUBLIC_SERVER_URL", "http://localhost:8090")
